...
def parse_args(argv=None):
    '''The command line: play the game, or `train` a learner on level 1.
    ...
    train = commands.add_parser('train', help='learn to beat level 1 over generations, with no window',
                                description='Learn to beat level 1 over generations of characters, '
                                            'with no window, printing a line per generation.')
...
def train(args):
    '''Train on level 1 as `args` says; whether a character beat it.'''
    ...
    return evolve.train(levels.LEVELS[0], args.settings, seed, args.generations) is not None
