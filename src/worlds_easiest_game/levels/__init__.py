...
COINS, OBSTACLES), plus INNER_WALLS if walls stand inside its course, then
append it to LEVELS below. Nothing in the engine or in the entry point has to
change. Place the level where the original has it on screen: every level
shares one TILE_SIZE checkerboard fixed to the canvas at the engine's
GRID_ORIGIN, so a level's position decides its tile colors, and the suite
checks every corner lands on a grid line.
